...
<head>
    ...
    <title>Nuevo Usuario | Curador Académico</title>
    ...
    <main class="ml-64 min-h-screen">
        <header class="w-full sticky top-0 z-40 bg-[#fbf8ff] dark:bg-slate-950 shadow-sm dark:shadow-none flex justify-between items-center px-12 py-6">
            <div class="flex flex-col">
                ...
                <p class="text-sm font-label text-outline">Creación de perfiles de acceso</p>
            </div>
        </header>
        ...
        <div class="px-12 py-8 bg-surface-container-low min-h-[calc(100vh-100px)] flex justify-center items-start">
            ...
            <div class="w-full max-w-xl bg-surface-container-lowest rounded-3xl p-8 shadow-sm ring-1 ring-outline-variant/15">
                <h3 class="text-xl font-extrabold text-primary mb-2">Crear Nuevo Usuario</h3>
                <p class="text-xs text-outline mb-6">Asigne los accesos y datos identificativos dentro de la plataforma.</p>

                <form method="POST" action="{{ route('superadministrador.usuarios.store') }}" class="space-y-5">
                    @csrf

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-outline mb-2 ml-1">Nombre Completo</label>
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Nombre completo" class="w-full border-none bg-surface-container-low rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary placeholder:text-outline/50" required>
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-outline mb-2 ml-1">Email Institucional</label>
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="correo@ejemplo.com" class="w-full border-none bg-surface-container-low rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary placeholder:text-outline/50" required>
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-outline mb-2 ml-1">DNI / Documento</label>
                        <input type="text" name="dni" value="{{ old('dni') }}" placeholder="Ingrese DNI" class="w-full border-none bg-surface-container-low rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary placeholder:text-outline/50" required>
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-outline mb-2 ml-1">Rol de Usuario</label>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-outline-variant/10 mt-6">
                        ...
                        <button type="submit" class="px-6 py-2.5 bg-[#001360] text-white rounded-xl font-semibold shadow-sm hover:bg-opacity-90 transition-colors">Guardar Usuario</button>
                    </div>
                </form>
            </div>
