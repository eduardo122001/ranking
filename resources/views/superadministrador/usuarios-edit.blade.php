...
<body class="bg-surface text-on-surface">
    ...
    <main class="ml-64 min-h-screen">
        <header class="w-full sticky top-0 z-40 bg-[#fbf8ff] dark:bg-slate-950 shadow-sm dark:shadow-none flex justify-between items-center px-12 py-6">
            ...
            <div>
                <form method="POST" action="{{ route('superadministrador.usuarios.destroy', $user->id) }}" onsubmit="return confirm('¿Está completamente seguro de eliminar este usuario? SI EL USUARIO TIENE NOTAS, TODAS SERAN BORRADAS !!!');">
                    @csrf
                    @method('DELETE')
